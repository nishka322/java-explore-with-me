...
public interface StatisticRepository extends JpaRepository<EndpointHitEntity, Long> {
    @Query("SELECT new ru.practicum.stats.dto.ViewStats(vs.app, vs.uri, COUNT(vs.ip)) " +
            "FROM EndpointHitEntity AS vs " +
    ...
    @Query("SELECT new ru.practicum.stats.dto.ViewStats(vs.app, vs.uri, COUNT(DISTINCT vs.ip)) " +
            "FROM EndpointHitEntity AS vs " +
            "WHERE vs.timestamp BETWEEN :start AND :end " +
            "GROUP BY vs.app, vs.uri " +
